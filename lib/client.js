/* Generated by scripts/build-client.mjs; edit client.source.js, config.js and modelsdev.js. */
...
const PROVIDER_TRANSFER_FORMAT = "dsh-custom-provider/providers";
const PROVIDER_TRANSFER_VERSION = 1;
...
function exportProviderBundle(view, routes) {
...
function parseProviderBundle(text) {
  let bundle;
...
function providerImportOperations(view, bundle, decisions) {
  const operations = [];
  for (const [route, profile] of Object.entries(bundle.providers)) {
...
function officialProviderOf(id) {
...
function findModelsDevMatch(catalog, id) {
...
function matchModelsDev(catalog, id) {
...
function modelsDevFields(metadata) {
...
function createModelsDevLoader(fetcher = (...args) => fetch(...args), clock = () => Date.now(), ttlMs = MODELS_DEV_TTL_MS) {
  let snapshot;
  let pending;
  return async () => {
...
async function enrichDiscoveredModels(found, loadPiAi) {
