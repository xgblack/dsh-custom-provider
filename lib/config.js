...
export function exportProviderBundle(view, routes) {
  const profiles = getAt(view.user, ["providers"]) ?? {};
  if (!isRecord(profiles) || !Array.isArray(routes) || !routes.length) throw new Error("Select at least one configured provider");
  const providers = {};
  const omittedHeaders = [];
  for (const route of routes) {
    if (!validTransferRoute(route) || !Object.hasOwn(profiles, route) || !isRecord(profiles[route])) {
      throw new Error(`Provider ${route} has no exportable user configuration`);
    }
    if (Object.hasOwn(providers, route)) throw new Error(`Provider ${route} was selected more than once`);
    const { headers, ...profile } = profiles[route];
...
export function parseProviderBundle(text) {
  ...
  for (const [route, profile] of Object.entries(bundle.providers)) {
    if (!validTransferRoute(route) || !isRecord(profile) || Object.hasOwn(profile, "headers")) {
...
export function providerImportOperations(view, bundle, decisions) {
  const operations = [];
  for (const [route, profile] of Object.entries(bundle.providers)) {
    ...
    operations.push({ op: "set", path: ["providers", route], value: profile });
  }
  return operations;
}
