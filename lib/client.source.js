...
function reasoningMode(value) {
  ...
  if (sameJson(value, DEFAULT_REASONING_LEVELS)) return "custom";
  return "custom";
}
...
function modelInputValue(mode, fallback) {
...
function reasoningValue(mode, levels, fallback) {
  if (mode === "inherit") return undefined;
...
function InputTypeControl({ draft, uid, disabled, onChange }) {
  const choices = [
    ["inherit", tr("继承默认配置", "Inherit default")],
    ["text", tr("文本", "Text")],
    ["image", tr("文本和图片", "Text and image")]
  ];
  choices.push(["custom", tr("自定义（高级 JSON）", "Custom (Advanced JSON)")]);
  return h("fieldset", { className: "dcp-choice-group" },
    h("legend", null, tr("输入类型", "Input types")),
    h("div", { className: "dcp-choice-options" }, choices.map(([value, label]) =>
      h("label", { className: "dcp-choice-option", key: value },
        h("input", { type: "radio", name: `${uid}-input`, value, checked: draft.input === value, disabled,
...
function ReasoningControl({ draft, uid, disabled, onChange }) {
  ...
  return h("div", { className: "dcp-field" },
    h("fieldset", { className: "dcp-choice-group" },
      ...
      h("div", { className: "dcp-choice-options" }, [
        ...
        h("input", { type: "radio", name: `${uid}-reasoning`, value, checked: draft.reasoning === value, disabled,
          onChange: () => onChange({ reasoning: value, reasoningLevels: value === "custom" && !draft.reasoningLevels.length
            ? reasoningLevels(undefined) : draft.reasoningLevels }) }), label)))),
    draft.reasoning === "custom" ? h("div", { className: "dcp-reasoning-levels" },
...
function previewModel(source, draft) {
  ...
  const input = modelInputValue(draft.input, source.input);
  if (input === undefined) delete next.input;
  else next.input = input;
  const reasoning = reasoningValue(draft.reasoning, draft.reasoningLevels, source.reasoningEfforts);
  ...
  const submit = async (restore = false) => {
    try {
      const changes = restore ? Object.fromEntries(COMMON_MODEL_FIELDS.map((key) => [key, undefined])) : {
        ...
        input: modelInputValue(draft.input, source.input),
        reasoningEfforts: reasoningValue(draft.reasoning, draft.reasoningLevels, source.reasoningEfforts)
      };
  ...
  return h("div", { className: "dcp-model-entry" },
    ...
    h("div", { className: "dcp-model-advanced", id: `${uid}-advanced`, hidden: !expanded },
      h("div", { className: "dcp-form-grid" },
        ...
        h(InputTypeControl, { draft, uid, disabled, onChange: (value) => change("input", value) }),
...
function CreateProvider({ view, schema, available, busy, api, onWrite, onClose, initialKind }) {
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      const found = response.value.map(discoveredModelEntry).filter(Boolean);
      ...
      kind === "catalog" ? h("div", { className: "dcp-field dcp-span-all" }, h("label", { htmlFor: "dcp-create-catalog" }, tr("提供方", "Provider")),
        ...
        h(React.Fragment, null,
          ...
          h("div", { className: "dcp-field dcp-span-all" },
            h("div", { className: "dcp-actions dcp-actions-start" },
  ...
  return h("div", { className: "dcp-provider-editor" },
    ...
    h("details", { className: "dcp-customized" },
      ...
      h("div", { className: "dcp-customized-body" },
        ...
        h(ModelCatalog, { view, route, mode, models, catalog, writable, busy: disabled || jsonDirty, onWrite,
...
function discoveredModelEntry(model) {
  ...
  if (Number.isSafeInteger(model.contextWindow) && model.contextWindow > 0) next.contextWindow = model.contextWindow;
  if (Number.isSafeInteger(model.maxTokens) && model.maxTokens > 0) next.maxTokens = model.maxTokens;
  if (Array.isArray(model.inputModalities)) next.input = [...model.inputModalities];
  else if (Array.isArray(model.input)) next.input = [...model.input];
...
function mergeDiscoveredModels(existing, discovered) {
  const byId = new Map(existing.filter((model) => typeof model?.id === "string").map((model) => [model.id, model]));
  const next = [];
  for (const model of discovered) {
    const entry = discoveredModelEntry(model);
    if (!entry) continue;
    next.push({ ...(byId.get(entry.id) ?? {}), ...entry });
  }
  return next;
}
...
function ModelConfigSection({ api, schema }) {
  ...
  const discover = React.useCallback(async (overrides = {}) => {
    ...
    try {
      ...
      if (!found.length) throw new Error(tr("提供方没有返回可用模型", "The provider returned no usable models"));
      setCatalogs((current) => ({ ...current, [currentRoute]: { status: "ready", models: found } }));
      if (currentMode === "listed") {
        const next = mergeDiscoveredModels(previous, found);
