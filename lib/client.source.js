...
function CreateProvider({ view, schema, available, busy, api, onWrite, onClose, onCredentialSaved, initialKind }) {
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(response.error.message);
      const enriched = await enrichDiscoveredModels(response.value, loadModelsDev, (provider) => loadPiAiCatalog(api, provider));
      if (!enriched.models.length) throw new Error(tr("提供方没有返回可用模型", "The provider returned no usable models"));
      setModels(enriched.models);
      setMetadataWarning(modelsDevWarning(enriched));
    } catch (error) { setFailure(error.message); }
  ...
  return h("div", { className: "dcp-form" },
    ...
    failure ? h("p", { className: "dcp-error", role: "alert" }, failure) : null,
    metadataWarning ? h("p", { className: "dcp-notice", role: "status" }, metadataWarning) : null,
    h("div", { className: "dcp-actions" },
...
function modelsDevWarning(result) {
  const piAiWarning = result.piAiError ? tr("pi-ai 目录查询失败", "pi-ai catalog lookup failed") + `: ${result.piAiError}` : "";
  if (!result.error) return piAiWarning;
  const prefix = result.stale
    ? tr("models.dev 更新失败，沿用缓存数据", "models.dev refresh failed; using cached data")
    : tr("models.dev 不可用，缺失字段未补全", "models.dev unavailable; missing fields were not filled");
  return [piAiWarning, `${prefix}: ${result.error}`].filter(Boolean).join("; ");
}
...
function ModelConfigSection({ api, schema }) {
  ...
  const discover = React.useCallback(async (overrides = {}) => {
    ...
    try {
      ...
      const enriched = await enrichDiscoveredModels(found, loadModelsDev,
        providerKind(currentView, currentRoute, directory) === "catalog" ? undefined : (provider) => loadPiAiCatalog(api, provider));
      setCatalogs((current) => ({ ...current, [currentRoute]: { status: "ready", models: enriched.models, warning: modelsDevWarning(enriched) } }));
