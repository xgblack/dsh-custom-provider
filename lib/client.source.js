...
const NS = NAMESPACE;
const LOCALE_NS = "settings.customProvider";
...
const inject = ["slots", "locale", "remote", "remote.settings", "remote.credentials", "remote.llm", "settingsSchema"];
let tr = (cn, en) => en;
const LOCALE_EN = {
...
// The host schema owns the reasoning level vocabulary; this copy only covers a schema it cannot read.
const REASONING_LEVEL_NAMES = ["off", "minimal", "low", "medium", "high", "xhigh", "max"];
...
.dcp-page-head { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; }
.dcp-transfer-toolbar { display: flex; flex-wrap: wrap; gap: 8px; }
.dcp-transfer { border-top: .5px solid var(--dcp-line); border-bottom: .5px solid var(--dcp-line); padding: 14px 0; display: flex; flex-direction: column; gap: 12px; }
.dcp-transfer-title { margin: 0; font-size: 14px; font-weight: 500; line-height: 22px; }
.dcp-transfer-list { list-style: none; padding: 0; margin: 0; max-height: 280px; overflow: auto; }
.dcp-transfer-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 12px; padding: 8px 0; border-bottom: .5px solid var(--dcp-line); font-size: 13px; }
.dcp-transfer-row:last-child { border-bottom: 0; }
.dcp-transfer-name { min-width: 0; overflow-wrap: anywhere; }
.dcp-transfer-name small { display: block; color: var(--dcp-muted); font-size: 12px; }
.dcp-transfer-row .dcp-select { width: auto; max-width: 120px; }
.dcp-transfer-file { font-size: 13px; max-width: 100%; }
.dcp-transfer-note { margin: 0; color: var(--dcp-muted); font-size: 12px; line-height: 18px; overflow-wrap: anywhere; }
...
.dcp-level-row { display: grid; grid-template-columns: 128px minmax(0, 1fr); gap: 8px; align-items: center; }
.dcp-level-name { display: inline-flex; align-items: center; gap: 6px; color: var(--dcp-fg); font-size: 13px; line-height: 22px; }
.dcp-level-name input { accent-color: var(--dsw-alias-brand-primary, #2b8063); margin: 0; }
...
@media (max-width: 560px) {
  [data-shortcut-modal="settings"]:has(.dcp-page) { flex-direction: column; }
  [data-shortcut-modal="settings"]:has(.dcp-page) > nav { width: 100%; height: 48px; padding: 0; flex: none; overflow: hidden; }
  [data-shortcut-modal="settings"]:has(.dcp-page) > nav > div:first-child { display: none; }
  [data-shortcut-modal="settings"]:has(.dcp-page) > nav > div:nth-child(2) { display: flex; flex-direction: row; align-items: center; gap: 4px; height: 48px; padding: 4px 8px; overflow-x: auto; }
  [data-shortcut-modal="settings"]:has(.dcp-page) > nav button { flex: none; white-space: nowrap; }
  [data-shortcut-modal="settings"]:has(.dcp-page) > div { width: 100%; min-width: 0; flex: 1; }
  [data-shortcut-modal="settings"]:has(.dcp-page) .dcp-page { width: 100%; }
  ...
  .dcp-level-row { grid-template-columns: minmax(0, 1fr); }
}
`;
...
// The host publishes the level vocabulary through the settings schema, so an upstream level
// change reaches this form without a plugin release; the built-in list is only a fallback.
function reasoningLevelNames(view, schema) {
  try {
    const node = schema.nodeAtPath(schema.rehydrate(view?.schema), ["providers", "\0probe", "models", "\0probe", "reasoningEfforts"]);
    const dict = node?.type === "union" ? node.list.find((entry) => entry.type === "dict") : node;
    const names = dict?.sKey?.type === "union" ? dict.sKey.list.map((entry) => entry.value) : undefined;
    const valid = names?.filter((name) => typeof name === "string" && name.length > 0);
    return valid?.length ? [...valid] : [...REASONING_LEVEL_NAMES];
  } catch { return [...REASONING_LEVEL_NAMES]; }
}

// Every host level is a row, in the host's escalation order. A key the host does not know
// cannot be a row; it is reported separately and dropped by the next save.
function reasoningDraft(value, names) {
  const configured = isRecord(value) ? value : undefined;
  const source = configured ?? DEFAULT_REASONING_EFFORTS;
  const levels = names.map((id) => ({
    id,
    enabled: Object.hasOwn(source, id),
    effort: Object.hasOwn(source, id) && source[id] != null ? String(source[id]) : ""
  }));
  const unknown = Object.entries(configured ?? {})
    .filter(([id]) => !names.includes(id))
    .map(([id, effort]) => ({ id, effort: effort == null ? "" : String(effort) }));
  return { levels, unknown };
}

function commonDraft(source, effective = source, names = REASONING_LEVEL_NAMES) {
  const reasoning = reasoningDraft(source.reasoningEfforts, names);
  return {
    ...
    reasoningUnknown: reasoning.unknown
  };
}
...
// The host schema accepts a dict no adapter can serve: an empty one, a non-off level with no
// wire value, an empty string, or `off` alone. Those fail when the provider loads, so the form
// refuses them before writing; the raw JSON editor runs the same check.
function reasoningEffortsFailure(efforts) {
  if (efforts === undefined || efforts === false) return "";
  if (!isRecord(efforts) || Object.keys(efforts).length === 0) {
    return tr("推理能力至少要声明一个等级；不推理请关闭该开关", "Declare at least one reasoning level; turn the switch off for a non-reasoning model");
  }
  const declared = Object.entries(efforts);
  for (const [id, wire] of declared) {
    if (wire === null) {
      if (id !== "off") return tr(`推理等级 ${id} 需要填写请求值`, `Reasoning level ${id} needs a request value`);
      continue;
    }
    if (typeof wire !== "string" || wire.trim().length === 0) {
      return tr(`推理等级 ${id} 的请求值不能为空`, `Reasoning level ${id} needs a non-empty request value`);
    }
  }
  if (!declared.some(([id]) => id !== "off")) {
    return tr("只声明 off 等于不推理；请再启用一个思考等级，或改为「不支持推理」",
      "Declaring only off means no reasoning; enable a thinking level or switch to “Reasoning unsupported”");
  }
  return "";
}
...
function ReasoningControl({ draft, uid, disabled, onChange }) {
  const levels = draft.reasoningLevels ?? [];
  const unknown = draft.reasoningUnknown ?? [];
  const updateLevel = (index, changes) => onChange({
    reasoningLevels: levels.map((entry, at) => at === index ? { ...entry, ...changes } : entry)
  });
  const failure = reasoningEffortsFailure(reasoningValue(draft.reasoningConfigured, draft.reasoning, levels));
  return h("div", { className: "dcp-field dcp-span-all" },
    h("label", { className: "dcp-toggle" },
    ...
    draft.reasoningConfigured ? h("fieldset", { className: "dcp-choice-group" },
      ...
      h("div", { className: "dcp-choice-options" }, [
        ...
        h("input", { type: "radio", name: `${uid}-reasoning`, value, checked: draft.reasoning === value, disabled,
          onChange: () => onChange({ reasoning: value }) }), label)))) : null,
    draft.reasoningConfigured && draft.reasoning === "custom" ? h("div", { className: "dcp-reasoning-levels" },
      levels.map((entry, index) => h("div", { className: "dcp-level-row", key: entry.id },
        h("label", { className: "dcp-level-name" },
          h("input", { type: "checkbox", className: "dcp-level-check", checked: entry.enabled, disabled,
            "aria-label": tr(`启用推理等级 ${entry.id}`, `Enable reasoning level ${entry.id}`),
            onChange: (event) => updateLevel(index, { enabled: event.target.checked }) }),
          h("span", null, entry.id)),
        h("input", { className: "dcp-input", value: entry.effort, disabled: disabled || !entry.enabled,
          placeholder: entry.id === "off" ? tr("留空 = 不发送该参数", "Blank = send nothing") : tr("请求值", "Request value"),
          "aria-label": tr(`推理等级 ${entry.id} 的请求值`, `Request value for reasoning level ${entry.id}`),
          onChange: (event) => updateLevel(index, { effort: event.target.value }) }))),
      unknown.length ? h("p", { className: "dcp-notice" },
        tr(`宿主不识别这些推理等级，保存时会移除：${unknown.map((entry) => entry.id).join("、")}`,
          `The host does not recognize these reasoning levels; saving removes them: ${unknown.map((entry) => entry.id).join(", ")}`)) : null,
      failure ? h("p", { className: "dcp-error", "aria-live": "polite" }, failure) : null) : null);
}
...
function ModelRow({ view, route, model, mode, schema, readOnly, busy, expanded, onToggle, onWrite, onDelete }) {
  const source = editableModel(view, route, model.id);
  const effective = model;
  readOnly = readOnly || mode === "inherited";
  const uid = React.useId();
  const sourceKey = JSON.stringify(source);
  const levelNames = React.useMemo(() => reasoningLevelNames(view, schema), [view?.schema, schema]);
  const initial = () => commonDraft(source, effective, levelNames);
  ...
  const importModelsDev = async () => {
    ...
    try {
      ...
      // A failed refresh must not fill the draft from an expired snapshot.
      ...
      setDraft((current) => ({
        ...
        ...(fields.contextWindow === undefined ? {} : { contextWindow: formatCapacity(fields.contextWindow) }),
        ...(fields.maxTokens === undefined ? {} : { maxTokens: formatCapacity(fields.maxTokens) }),
        ...(fields.input === undefined ? {} : { inputConfigured: true, inputTypes: [...fields.input] })
      }));
      setDirty(true);
      setImportStatus(tr("已填入 models.dev 数据，点击「保存模型」提交", "models.dev data filled in; click Save model to apply") + `: ${match.key}`);
    } catch (error) { setFailure(error instanceof Error ? error.message : String(error)); }
    finally { setImporting(false); }
  };
  const levelFailure = reasoningEffortsFailure(reasoningValue(draft.reasoningConfigured, draft.reasoning, draft.reasoningLevels));
  const submit = async (restore = false) => {
    setImportStatus("");
    try {
      if (!restore && levelFailure) throw new Error(levelFailure);
  ...
  return h("div", { className: "dcp-model-entry" },
    ...
    !readOnly && (expanded || dirty) ? h("div", { className: "dcp-actions" },
      ...
      h("button", { type: "button", className: "dcp-button dcp-button-primary dcp-button-small", disabled: disabled || !dirty || Boolean(levelFailure),
        title: levelFailure || undefined,
...
function AdvancedEditor({ source, revision, readOnly, busy, save, onDirtyChange }) {
  ...
  const submit = async () => {
    try {
      const value = parseModelJson(draft, source.id);
      const invalidReasoning = reasoningEffortsFailure(value.reasoningEfforts);
      if (invalidReasoning) throw new Error(invalidReasoning);
...
function ModelCatalog({ view, route, schema, mode, models, catalog, writable, busy, onWrite, onDiscover }) {
  ...
  return h("section", { className: "dcp-model-catalog", "aria-label": tr("模型", "Models") },
    ...
    shownModels.length ? h("div", { className: "dcp-model-list" }, shownModels.map((model) => h(ModelRow, { key: model.id, view, route, model, mode, schema,
  ...
  return h("div", { className: "dcp-provider-editor" },
    ...
    h("details", { className: "dcp-customized" },
      ...
      h("div", { className: "dcp-customized-body" },
        ...
        h(ModelCatalog, { view, route, schema, mode, models, catalog, writable, busy: disabled, onWrite,
...
function ProviderTransfer({ mode, view, schema, writable, busy, onWrite, onClose }) {
  const exportable = Object.entries(getAt(view.user, ["providers"]) ?? {}).filter(([, profile]) => isRecord(profile));
  const [selected, setSelected] = React.useState(() => new Set(exportable.map(([id]) => id)));
  ...
  const chooseFile = async (event) => {
    ...
    event.target.value = "";
    if (!file) return;
    setFailure(""); setStatus(""); setBundle(undefined);
    setWorking(true);
    try {
      if (file.size > 5 * 1024 * 1024) throw new Error(tr("配置文件超过 5 MB", "Configuration file exceeds 5 MB"));
      const parsed = parseProviderBundle(await file.text());
      setDecisions(Object.fromEntries(Object.keys(parsed.providers).map((id) =>
        [id, hasAt(view.value, ["providers", id]) ? "skip" : "add"])));
  ...
  const download = () => {
    ...
    try {
      const data = exportProviderBundle(view, [...selected]);
      const url = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2) + "\n"], { type: "application/json" }));
      const link = document.createElement("a");
      link.href = url; link.download = "dsh-providers.json";
      document.body.append(link); link.click(); link.remove();
      setTimeout(() => URL.revokeObjectURL(url), 1000);
      setStatus(tr(`已准备导出 ${selected.size} 个提供方`, `Prepared ${selected.size} providers for export`));
    } catch (error) { setFailure(error.message); }
  };
  let operations = [];
  ...
  if (bundle) {
    try {
      operations = providerImportOperations(view, bundle, decisions);
      if (operations.length) {
        const invalid = schema.validate(schema.rehydrate(view.schema), candidate(view, operations));
        if (invalid) validationFailure = typeof invalid === "string" ? invalid : JSON.stringify(invalid);
      }
    } catch (error) { validationFailure = error.message; }
  }
  const stale = bundle && revision !== view.revision;
  const importSelected = async () => {
    if (!operations.length || stale || validationFailure) return;
    setFailure(""); setStatus(""); setWorking(true);
    try {
      const error = await onWrite(operations, revision);
      if (error) throw new Error(error);
      setStatus(tr(`已导入 ${operations.length} 个提供方；请核对密钥引用并重新配置缺失密钥`,
        `Imported ${operations.length} providers; review credential references and configure missing keys`));
      setBundle(undefined);
    } catch (error) { setFailure(error.message); }
    finally { setWorking(false); }
  };
  return h("section", { className: "dcp-transfer", "aria-label": mode === "export" ? tr("导出提供方", "Export providers") : tr("导入提供方", "Import providers") },
    h("h3", { className: "dcp-transfer-title" }, mode === "export" ? tr("导出提供方配置", "Export provider configuration") : tr("导入提供方配置", "Import provider configuration")),
    mode === "export" ? h(React.Fragment, null,
      exportable.length ? h("ul", { className: "dcp-transfer-list" }, exportable.map(([id, profile]) => h("li", { className: "dcp-transfer-row", key: id },
        h("label", { className: "dcp-transfer-name" },
          h("input", { type: "checkbox", checked: selected.has(id), disabled: busy, onChange: (event) => setSelected((current) => {
            const next = new Set(current); if (event.target.checked) next.add(id); else next.delete(id); return next;
          }) }), " ", profile.displayName || id, h("small", null, id)),
        Object.hasOwn(profile, "headers") ? h("span", { className: "dcp-transfer-note" }, tr("请求头不导出", "Headers excluded")) : null))) :
        h("p", { className: "dcp-empty" }, tr("没有用户配置的提供方可导出", "No user-configured providers to export")),
      h("p", { className: "dcp-transfer-note" }, tr("不导出凭据值和自定义请求头。密钥引用会保留；接口地址也可能包含敏感参数。", "Credential values and custom headers are excluded. Credential references remain; endpoint URLs may contain sensitive parameters."))) :
      h(React.Fragment, null,
        h("label", { className: "dcp-field" }, tr("配置文件（JSON）", "Configuration file (JSON)"),
          h("input", { className: "dcp-transfer-file", type: "file", accept: ".json,application/json", disabled: !writable || busy || working, onChange: chooseFile })),
        bundle ? h("ul", { className: "dcp-transfer-list" }, Object.entries(bundle.providers).map(([id, profile]) => {
          const exists = hasAt(view.value, ["providers", id]);
          return h("li", { className: "dcp-transfer-row", key: id },
            h("span", { className: "dcp-transfer-name" }, profile.displayName || id,
              h("small", null, id, exists ? tr(" · 已存在", " · exists") : tr(" · 新增", " · new"),
                bundle.omittedHeaders.includes(id) ? tr(" · 请求头未包含", " · headers excluded") : "")),
            h("select", { className: "dcp-select", value: decisions[id] ?? "skip", disabled: busy || working || !!stale,
              "aria-label": tr(`${id} 的导入方式`, `Import action for ${id}`),
              onChange: (event) => setDecisions((current) => ({ ...current, [id]: event.target.value })) },
              exists ? [h("option", { key: "skip", value: "skip" }, tr("跳过", "Skip")),
                h("option", { key: "replace", value: "replace" }, tr("覆盖", "Replace"))] :
                [h("option", { key: "add", value: "add" }, tr("导入", "Import")),
                  h("option", { key: "skip", value: "skip" }, tr("跳过", "Skip"))]));
        })) : null,
        bundle ? h("p", { className: "dcp-transfer-note" }, tr("覆盖会替换目标的用户层配置；继承层仍由目标环境决定。密钥值不会导入，已有的同名密钥引用可能在目标环境生效。", "Replace overwrites target user settings; inherited settings depend on the target. Secret values are not imported, but an existing credential with the same reference may be used.")) : null),
    stale ? h("p", { className: "dcp-error", role: "alert" }, tr("配置已变化，请重新选择文件并预览", "Settings changed; choose the file again to preview")) : null,
    validationFailure ? h("p", { className: "dcp-error", role: "alert" }, validationFailure) : null,
    failure ? h("p", { className: "dcp-error", role: "alert" }, failure) : null,
    status ? h("p", { className: "dcp-status", role: "status" }, status) : null,
    h("div", { className: "dcp-actions" },
      h("button", { type: "button", className: "dcp-button", disabled: working, onClick: onClose }, tr("关闭", "Close")),
      mode === "export" ? h("button", { type: "button", className: "dcp-button dcp-button-primary", disabled: busy || !selected.size, onClick: download }, tr("下载 JSON", "Download JSON")) :
        bundle ? h("button", { type: "button", className: "dcp-button dcp-button-primary", disabled: !writable || busy || working || !!stale || !!validationFailure || !operations.length,
          onClick: importSelected }, working ? tr("导入中", "Importing") : tr(`导入所选 ${operations.length} 项`, `Import ${operations.length} selected`)) : null));
}

function ModelConfigSection({ api, schema, translate }) {
  tr = translate;
  ...
  return h("div", { className: "dcp-page" },
    h("div", { className: "dcp-page-head" },
      h("h2", null, tr("模型配置", "Model configuration")),
      h("div", { className: "dcp-transfer-toolbar" },
        h("button", { type: "button", className: "dcp-button dcp-button-small", disabled: busy,
          onClick: () => setTransferMode((current) => current === "import" ? "" : "import") }, tr("导入配置", "Import configuration")),
        h("button", { type: "button", className: "dcp-button dcp-button-small", disabled: busy,
          onClick: () => setTransferMode((current) => current === "export" ? "" : "export") }, tr("导出配置", "Export configuration")))),
    ...
    transferMode ? h(ProviderTransfer, { key: transferMode, mode: transferMode, view, schema, writable, busy,
      onWrite: saveOperation, onClose: () => setTransferMode("") }) : null,
...
function apply(ctx) {
  ...
  ctx.effect(() => ctx.locale.register(LOCALE_NS, { zh: LOCALE_ZH, en: LOCALE_EN }), "dsh-custom-provider: dictionaries");
  ...
  ctx.slots.inject("settings.section", () => ctx.slots.register({
    ...
    label: () => translate("nav"),
    locale: LOCALE_NS,
    inject: () => ({ api, schema: ctx.settingsSchema, translate })
  }, ModelConfigSection));
  ctx.effect(() => {
