...
function ModelRow({ view, route, model, mode, readOnly, busy, expanded, onToggle, onWrite, onDelete }) {
  ...
  const lastModel = mode === "listed" && getAt(view.user, ["providers", route, "models"]).length === 1;
  ...
  return h("div", { className: "dcp-model-entry" },
    h("div", { className: "dcp-model-row" },
      ...
      h("div", { className: "dcp-model-tools" },
        ...
        mode === "listed" ? h("button", { type: "button", className: "dcp-button dcp-icon-button dcp-icon-button-danger", disabled: disabled || lastModel,
          title: lastModel ? tr("至少保留一个模型", "Keep at least one model") : tr("删除模型", "Remove model"),
    ...
    h("div", { className: "dcp-model-advanced", id: `${uid}-advanced`, hidden: !expanded },
      h("div", { className: "dcp-form-grid" },
        field("context", tr("上下文窗口", "Context window"),
          h("input", { id: `${uid}-context`, className: "dcp-input", type: "number", min: "1", step: "1", value: draft.contextWindow, disabled,
            placeholder: effective.contextWindow === undefined ? tr("继承默认值", "Inherit default") : String(effective.contextWindow),
            onChange: (event) => change("contextWindow", event.target.value) })),
        field("output", tr("输出上限", "Output limit"),
          h("input", { id: `${uid}-output`, className: "dcp-input", type: "number", min: "1", step: "1", value: draft.maxTokens, disabled,
            placeholder: effective.maxTokens === undefined ? tr("继承默认值", "Inherit default") : String(effective.maxTokens),
            onChange: (event) => change("maxTokens", event.target.value) })),
        field("reasoning", tr("推理能力", "Reasoning"),
          h("select", { id: `${uid}-reasoning`, className: "dcp-select", value: draft.reasoning, disabled, onChange: (event) => change("reasoning", event.target.value) },
            ...
            draft.reasoning === "custom" ? h("option", { value: "custom" }, tr("自定义档位（JSON）", "Custom levels (JSON)")) : null)),
        field("input", tr("输入类型", "Input types"),
          h("select", { id: `${uid}-input`, className: "dcp-select", value: draft.input, disabled, onChange: (event) => change("input", event.target.value) },
            h("option", { value: "inherit" }, tr("继承提供方或目录", "Inherit provider or catalog")),
            h("option", { value: "text" }, tr("文本", "Text")),
            h("option", { value: "image" }, tr("文本和图片", "Text and image")),
            draft.input === "custom" ? h("option", { value: "custom" }, tr("自定义（JSON）", "Custom (JSON)")) : null)))),
    ...
    !readOnly && (expanded || dirty) ? h("div", { className: "dcp-actions" },
      expanded ? h("button", { type: "button", className: "dcp-button dcp-button-small", disabled: busy, onClick: () => submit(true) }, tr("恢复继承", "Restore inherited")) : null,
    ...
    h("details", { className: "dcp-customized", hidden: !expanded },
      ...
      h("div", { className: "dcp-customized-body" }, h(AdvancedEditor, { source, revision: view.revision, readOnly, busy: busy || dirty,
        save: (next, revision) => { try { return onWrite(replaceModelOperation(view, route, model.id, next), revision); }
          catch (error) { return Promise.resolve(error.message); } } }))));
}

function AdvancedEditor({ source, revision, readOnly, busy, save }) {
  const sourceKey = JSON.stringify(source);
...
function CreateProvider({ view, schema, available, busy, api, onWrite, onClose, initialKind }) {
  const kind = initialKind ?? (available.length ? "catalog" : "custom");
  ...
  return h("div", { className: "dcp-form" },
    h("h3", { className: "dcp-editor-title" }, kind === "catalog" ? tr("添加内置提供方", "Add built-in provider") : tr("添加自定义提供方", "Add custom provider")),
    h("div", { className: "dcp-form-grid" },
      kind === "catalog" ? h("div", { className: "dcp-field dcp-span-all" }, h("label", { htmlFor: "dcp-create-catalog" }, tr("提供方", "Provider")),
        ...
        h(React.Fragment, null,
          ...
          h("div", { className: "dcp-field" }, h("label", { htmlFor: "dcp-create-model" }, tr("首个模型 ID", "First model ID")),
            h("input", { id: "dcp-create-model", className: "dcp-input", value: modelId, disabled, placeholder: "model-id", onChange: (event) => setModelId(event.target.value) }))),
      h("div", { className: "dcp-field dcp-span-all" }, h("label", { htmlFor: "dcp-create-key" }, "API Key"),
...
function ProviderEditor({ view, route, schema, api, writable, busy, onWrite, models, mode, catalog, onDiscover, onCredentialSaved }) {
  ...
  return h("div", { className: "dcp-provider-editor" },
    h("div", { className: "dcp-field" },
      ...
      h("div", { className: "dcp-key-row" },
        ...
        h("button", { type: "button", className: "dcp-button dcp-button-small", disabled: disabled || !key || jsonDirty || credential?.writable === false,
      ...
      h("div", { className: "dcp-customized-body" },
        h("div", { className: "dcp-form-grid" },
          connectionField("name", tr("显示名称", "Display name"),
            h("input", { id: `dcp-provider-name-${route}`, className: "dcp-input", value: draft.displayName, disabled: disabled || jsonDirty,
          ...
          connectionField("api", tr("协议", "Protocol"),
            h("select", { id: `dcp-provider-api-${route}`, className: "dcp-select", value: draft.api, disabled: disabled || jsonDirty,
          ...
          connectionField("url", tr("接口地址", "Endpoint"),
            h("input", { id: `dcp-provider-url-${route}`, className: "dcp-input", type: "url", value: draft.baseURL, disabled: disabled || jsonDirty,
              placeholder: effective.baseURL || tr("使用目录默认地址", "Catalog endpoint"), onChange: (event) => change("baseURL", event.target.value) }), true)),
        h("div", { className: "dcp-actions" },
          h("button", { type: "button", className: "dcp-button dcp-button-primary dcp-button-small", disabled: disabled || !dirty || jsonDirty, onClick: saveCommon }, tr("保存设置", "Save settings"))),
        h(ModelCatalog, { view, route, mode, models, catalog, writable, busy: disabled || jsonDirty, onWrite, onDiscover }),
        h("details", { className: "dcp-customized" },
          h("summary", { className: "dcp-customized-summary" }, tr("提供方高级 JSON", "Advanced provider JSON")),
          h("div", { className: "dcp-customized-body" },
            h("textarea", { className: "dcp-json", value: json, disabled: disabled || dirty, spellCheck: false, "aria-label": tr("当前提供方 JSON", "Current provider JSON"),
            ...
            h("div", { className: "dcp-actions" },
              ...
              h("button", { type: "button", className: "dcp-button dcp-button-primary dcp-button-small", disabled: disabled || !jsonDirty || dirty, onClick: saveJson }, tr("保存 JSON", "Save JSON"))))))),
    failure ? h("p", { className: "dcp-error", role: "alert" }, failure) : null,
...
function ModelConfigSection({ api, schema }) {
  ...
  return h("div", { className: "dcp-page" },
    h("h2", null, tr("模型配置", "Model configuration")),
    h("p", { className: "dcp-intro" }, tr("配置提供方的连接、认证与模型参数。", "Configure provider connections, credentials, and model settings.")),
    ...
    providers.length ? h("ul", { className: "dcp-provider-list" }, providers.map((provider) => {
      ...
      return h("li", { key: provider.id, className: "dcp-provider-card" },
        ...
        open ? h(ProviderEditor, { key: provider.id, view, route: provider.id, schema, api, writable, busy, onWrite: saveOperation,
          ...
          onCredentialSaved: (ref) => setCredentialStates((current) => ({ ...current, [ref]: { configured: true, writable: true } })) }) : null);
      ...
      h("div", { className: "dcp-form" },
        h("div", { className: "dcp-add-actions" },
          h("button", { type: "button", className: "dcp-button dcp-add-option", disabled: !writable || busy || available.length === 0,
            onClick: () => { setAddingProvider("catalog"); setFailure(""); } }, h(Icon, { kind: "plus" }), tr("添加内置提供方", "Add built-in provider")),
          h("button", { type: "button", className: "dcp-button dcp-add-option", disabled: !writable || busy,
            onClick: () => { setAddingProvider("custom"); setFailure(""); } }, h(Icon, { kind: "plus" }), tr("添加自定义提供方", "Add custom provider")))));
}
