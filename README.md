...
- **Providers** can be activated from the installed catalog or declared with a route ID, endpoint, and protocol. A new custom provider starts with an empty model list; use **Fetch available models** to populate it, then remove models that are not needed. Common fields are display name, endpoint, and protocol; provider Advanced JSON edits only that route's user override. A provider defined in the composition cannot be removed here.
...
- **Common** exposes model name, reasoning capability, input types, context window, and output limit. Empty values inherit from the provider or installed catalog. Capacity fields accept values such as `256K` and `1M`. Input and reasoning choices use single-select controls; custom reasoning starts with editable `off`, `low`, `high`, and `max` levels. Advanced JSON previews the current common-field draft in real time, while protocol-specific fields remain editable there.
